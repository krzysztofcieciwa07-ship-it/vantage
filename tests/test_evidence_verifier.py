...
def test_independent_verifier_passes_for_current_evidence(tmp_path):
    evidence = tmp_path / "valid.txt"
    make_valid_evidence(evidence)
...
def test_tampered_evidence_is_rejected(tmp_path):
    ...
    evidence.write_text(
        text.replace("afe_vantage_sha256=", "afe_vantage_sha256=0", 1),
        encoding="utf-8",
    )
    result = run_verifier(evidence)
    assert result.returncode != 0
...
def test_false_commit_provenance_is_rejected(tmp_path):
    evidence = tmp_path / "false-commit.txt"
    make_valid_evidence(evidence)
    text = evidence.read_text(encoding="utf-8")
    current = os.environ.get("GITHUB_SHA", "test-commit")
    evidence.write_text(
        text.replace("commit=" + current, "commit=FALSE_COMMIT", 1),
        encoding="utf-8",
    )
    result = run_verifier(evidence)
    assert result.returncode != 0
